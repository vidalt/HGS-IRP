...
void Client::computeJourSuiv () 
{
	...
	// donnant computeJourSuiv[codeT][jourT] : peut on rajouter la tache assimil�e 
	...
	// on remplace les 0 et les 1 par la donn�e du prochain jour o� l'on peut
	// ajouter la tache

	for (int k=0 ; k <= (int)pow((float)2,(int)nbJours) ; k++)
	{
		// une petite marque dans le champ 0 qui �tait vide a �t� laiss�e
		// si cette valeur vaut -1 il n'y a rien � voir.
		if (jourSuiv[k][0] != -1)
		{

			// on parcourt de droite � gauche pour trouver le premier element non nul
			day = 0 ;
			j = (int)nbJours ;
			while ( day == 0 )
			{
