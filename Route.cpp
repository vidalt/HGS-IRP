...
// pour un client donné, trouve la meilleure position d'insertion et sa quantité
void Route::evalInsertClient (Noeud * U) 
{
	...
	else // U est déja placé dans notre route
	{
		...
		// temporairement on va enlever le noeud de la liste chainée (en O(1))
		U->pred->suiv = U->suiv ;
		U->suiv->pred = U->pred ;
		courNoeud = depot ;

		// et parcourir la route à nouveau
		while (!courNoeud->estUnDepot || firstIt == true )
		{
			firstIt = false ;
			cout = params->timeCost[courNoeud->cour][U->cour] 
			...
			// au final on peut placer d'une meilleure manière
			if (cout < bestInsertion[U->cour].detour - 0.0001)
			{ 
				bestInsertion[U->cour].detour = cout ;
				bestInsertion[U->cour].place = courNoeud ;
