...
Route::Route(int cour, int day, Noeud *depot, double temps, double charge, double maxRouteTime, double vehicleCapacity, Params *params, LocalSearch *myLS) : cour(cour), day(day), depot(depot), temps(temps), charge(charge), maxRouteTime(maxRouteTime), vehicleCapacity(vehicleCapacity), params(params), myLS(myLS)
{
...
void Route::updateRouteData()
{
	...
	if (charge < vehicleCapacity + 0.0001 && earlT < maxRouteTime + 0.0001)
		isFeasible = true;
	else
		isFeasible = false;
...
void Route::evalInsertClient(Noeud *U)
{
	...
	else // U est d�ja plac� dans notre route
	{
		bestInsertion[U->cour].load = std::max<double>(0., vehicleCapacity + myLS->demandPerDay[day][U->cour] - charge);
		bestInsertion[U->cour].detour = params->timeCost[U->pred->cour][U->cour] - params->timeCost[U->pred->cour][U->suiv->cour] + params->timeCost[U->cour][U->suiv->cour];
		bestInsertion[U->cour].place = U->pred;

		// cependant on va regarder si il n'y a pas une meilleure insertion possible
		// temporairement on va enlever le noeud de la liste chain�e (en O(1))
		U->pred->suiv = U->suiv;
		U->suiv->pred = U->pred;
		courNoeud = depot;
